...
def scaled_dot_product_attention(V: Tensor, K: Tensor, Q: Tensor, mask: Tensor) -> Tensor:
    ...
    scaled = similarities / torch.sqrt(torch.tensor(d_k, dtype=torch.float32))
        
    if mask is not None:
        mask = mask[:, :, None, None] 
        
        # Apply the mask
        masked = scaled.masked_fill(mask == 1, NEG_INF)
    else:
        masked = scaled
...
class MultiHeadAttenionLayer(nn.Module):
    def __init__(self, info: ModelInfo, num_heads: int):
        ...
        self.W_v = LinearLayer(info, d_model, bias=False)
        self.W_q = LinearLayer(info, d_model, bias=False)
        self.W_k = LinearLayer(info, d_model, bias=False)
        self.W_o = LinearLayer(info, d_model, bias=False)
